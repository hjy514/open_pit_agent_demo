...
9. 不得把合成风险数据称为真实矿山监测，不得把预设安全中间点分段导航称为通用路网边级动态绕行，不得把有界经验记忆称为已训练大模型。
10. 改动后至少运行：
...
- 策略层道路限制登记和任务准入限制；
- 矿山综合场景红色风险下，CARLA应急任务执行“安全中间点→处置终点”两段真实导航；
- 完成任务后车辆驶离共享车道并待命；
- 所有任务结束后物理制动和低速终态确认；
...
- 矿山地图 `0325_5` 和3辆 `vehicle.cat.cat` 矿卡已能综合运行；
- 增加8个固定监测站、3辆移动装备数据和实时安全闭环面板；
- 修复 `/monitoring` 接口和旧 API 进程兼容问题；
- 场景明确为“强降雨诱发东帮边坡失稳”，加入触发证据和处置顺序；
- 红色风险后封控危险路段，授权任务按预设安全中间点分段导航；
- 53项自动测试通过，综合 Mock 通过；新增绕行功能等待用户执行 CARLA 回归。
